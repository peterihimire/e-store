...
public class CategoryController {
  ...
  public ResponseEntity<ApiResponse> getAllCategories(){
    try {
      ...
      return ResponseEntity.ok(new ApiResponse("success","Categories returned", categories));
    } catch (Exception e) {
      return  ResponseEntity.status(INTERNAL_SERVER_ERROR).body(new ApiResponse("fail","Error:", INTERNAL_SERVER_ERROR));
    }
  }
  ...
  public ResponseEntity<ApiResponse> addCategory(@RequestBody Category name){
    try {
      ...
      return ResponseEntity.ok(new ApiResponse("success","success", category));
  ...
  public ResponseEntity<ApiResponse> updateCategory(@PathVariable String slug, @RequestBody Category category){
    try {
     Category data = categoryService.updateCategory(category, slug);
      return ResponseEntity.ok(new ApiResponse("success","Update success", data));
    } catch (ResourceNotFoundException e) {
      return ResponseEntity.status(NOT_FOUND).body(new ApiResponse("fail",e.getMessage(), null));
    }
  }
