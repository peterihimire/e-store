...
public class OrderProcessingEmailBuilder {
  ...
  public EmailRequest build(Order order) {
    ...
    attachment.setFilename("Processing-" + order.getOrderNumber()+ ".pdf");
    attachment.setContentType("application/pdf");
    attachment.setContent(pdf);

    EmailRequest request = new EmailRequest();
    request.setTo(order.getUser().getEmail());
    request.setSubject("Order Processing");
